...
import { DollarOutlined, DownSquareOutlined, StarOutlined
...
// 2. load products on user search input
    ...
    //show categorie in a list of checkbox
    const showCategories = () => categories.map((c) => 
        <div key={c._id}>  
            <Checkbox onChange={handleCheck} className="pb-2 pl-4 pr-4" value={c._id} checked={categoryIds.includes(c._id)} name="category">
                {c.name}
            </Checkbox>
            <br /><br />
        </div>)
    ...
    //5.show products with star rating
    ...
    //7.shor products based on brand nam(e
    const showBrands = () => 
        brands.map((b) => (
            <Radio 
                value={b} 
